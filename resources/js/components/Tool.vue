            ...
            <div class="p-3 flex items-center justify-between border-b border-50">
                <div class="w-full flex flex-wrap">
                    <div class="w-2/3 flex flex-wrap justify-start">
                        ...
                        <label v-if="buttons.upload_button && !loadingfiles" class="manual_upload cursor-pointer">
                        ...
                        <button v-if="view == 'list'" @click.stop.prevent="viewAs('grid')" class="btn btn-default btn-small btn-primary text-white mr-3">
                        ...
                        <button v-if="view == 'grid'" @click.stop.prevent="viewAs('list')" class="btn btn-default btn-small btn-primary text-white mr-3">
                        ...
                        <button v-if="buttons.select_multiple" :disabled="isCheckAll" type="button" class="btn btn-default btn-primary flex items-center text-white mr-3 px-3"  @click.stop.prevent="multiSelecting=!multiSelecting">
                        ...
                        <button type="button" class="btn btn-default btn-primary flex items-center text-white mr-3 px-3"  v-on:click.stop.prevent="checkAll()">
                            <svg class="w-6 h-6 fill-current pt-1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><circle cx="6.5" cy="6.75" r="1.5"/><path d="M17.75 10.25A6.25 6.25 0 1024 16.5a6.257 6.257 0 00-6.25-6.25zm3.163 5.028L18.13 18.99a1.46 1.46 0 01-1.076.583h-.107a1.454 1.454 0 01-1.035-.434l-1.435-1.436a.75.75 0 011.06-1.06l1.234 1.234a.251.251 0 00.2.072.247.247 0 00.182-.1l2.563-3.475a.751.751 0 111.2.9z"/><path d="M0 3.25v7a1.981 1.981 0 001.957 2h7.858a1 1 0 000-2H2.5a.5.5 0 01-.5-.5l-.037-6a.5.5 0 01.5-.5H21.5a.5.5 0 01.5.5v3.5a1 1 0 002 0v-4a1.981 1.981 0 00-1.956-2H1.957A1.982 1.982 0 000 3.25z"/><circle cx="12.5" cy="6.75" r="1.5"/></svg>
                            <span class="ml-2 text-sm">tout</span>
                        </button>
                        <button v-if="multiSelecting && selectedFiles.length > 0" type="button" class="btn btn-default btn-small btn-danger text-white mr-3" @click.stop.prevent="openMultiDeleteModal">
                    ...
                    <div class="w-1/3 flex flex-wrap justify-end">
                        <div class="relative w-1/2 max-w-xs pr-3">
                            <template v-if="showFilters">
                                <select class="pl-search form-control form-input form-input-bordered w-full" v-model="filterBy" @change.stop.prevent="filterFiles">
        ...
        methods: {
            ...
            checkAll() {
                ...
                if (this.isCheckAll){
                    ...
                    {
                        const findIndex = _.findIndex(this.selectedFiles, this.files[file]);

                        if (findIndex >= 0) {
                            this.selectedFiles.splice(findIndex, 1);
                            return;
                        }
                        if (this.selectedFiles.indexOf(this.files[file]) === -1) {
                            this.selectedFiles.push(this.files[file]);
                        }
                    }
                } else {
                    this.selectedFiles = [];
